...
def signal_from_indicators(df):
    ...
    # slope_k = slope(df['K']) # Unused
    # slope_d = slope(df['D']) # Unused

    buy  = (k_now > d_now) and (cci_now < -100)
    sell = (k_now < d_now) and (cci_now > 100)
...
def main():
    ...
    NOTES_COL = 'notes'
    BASE_COLS = ['datetime', 'signal', 'token', 'close price', 'CCI', 'stoch K', 'stoch D', 'slope K', 'slope D']
